-- ═══════════════════════════════════════════════════════════════════════
--  ⚠️  АРХИВ — история решений, НЕ источник правды (с 2026-07-17, этап 4).
--
--  Актуальное состояние схемы БД лежит в снимке:
--    supabase/migrations/20260717120000_baseline.sql
--  Этот файл — append-only летопись: тут функции по 3-4 версии подряд
--  (complete_lesson, start_lesson...), и что реально в базе — из него не видно.
--  Оставлен ради комментариев-решений «почему так сделано».
--
--  Новые изменения БД: НЕ дописывать сюда, а создавать новый файл миграции
