...
| `CanvasBoard.jsx` | Рабочая область: ноды позиционируются абсолютно, SVG-слой для линий, пан холста, кнопка «+ Нода». Локальный черновик в localStorage (ключ — `canvasStorageKeys.js`) не даёт потерять несохранённые правки при перезагрузке — чистится в `CanvasPage.handleSave` после успешного сохранения; если черновик короче того, что реально на сервере (правили в другом месте) — при монтировании спрашивает подтверждение вместо молчаливой подмены. `forwardRef`+`useImperativeHandle`: `clearAll()`/`focusStart()` для кнопок шапки CanvasPage.jsx (nodes/offset/scale живут здесь, не в CanvasPage). Средняя кнопка мыши — панорамирование (даже начатое над нодой); левая по пустому месту — рамка выделения (useCanvasSelection.js), протяжка любой ноды из выделения двигает всю группу. Ради мемоизации CanvasNode.jsx/CanvasConnections.jsx: `handleNodeMouseDown`/`toWorld`/`startPortDrag` — `useCallback`, `EMPTY_NODES` — стабильная пустая ссылка для `allNodes` у mini/nano нод. `boardRectRef` — кэш `getBoundingClientRect()` холста (ResizeObserver + resize), не вызывается заново на каждом wheel/mousemove — сам вызов форсирует синхронный layout браузера, на потоке событий зума/протяжки это давало рваное, дёрганое движение |
