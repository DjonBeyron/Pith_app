...
| Файл | Зачем нужен |
|------|-------------|
| `CanvasPage.jsx` | Обёртка страницы: шапка с кнопками «⚙», «← Назад», «▶» (открывает плеер), «↻ С сервера», «Очистить»/«В начало» (дотягиваются до CanvasBoard через ref), тройка «Сохранить → Граф → Продакшен»; запускает `LessonPlayer` поверх редактора. handleSave ловит ошибку сохранения и явно алертит админа (не молчит, если RLS/сеть не дали записать). `linkableLessons` — `useMemo` (не новый массив на каждый рендер) — идёт пропом до каждой CanvasNode.jsx (React.memo), нестабильная ссылка срывала бы мемоизацию у всех нод разом |
