...
| `useSequentialPreload.js` | Качает файлы по одному строго по очереди (не параллельно, не дальше чем на несколько сообщений вперёд от показанного — см. `allowUpTo`), с авто-повтором при обрыве; держит в памяти буфер из 5 последних видео/аудио (FIFO — новый вытесняет самый старый), фото никогда не выгружает, видео при выгрузке оставляет один кадр-превью; умеет подгрузить файл повторно по требованию |
