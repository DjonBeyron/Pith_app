...
| `modules/reaction/ReactionModule.jsx` | Реакция на сообщение: своего пузыря нет — эмодзи порталом уезжает ВНУТРЬ пузыря соседнего сообщения (вниз слева), пузырь под неё подрастает сам; `target` — к ответу ученика (пузырь справа) или к реплике учителя (слева); строка ноды в ленте пустая, нулевой высоты |
