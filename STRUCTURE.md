...
| `useCanvasBoardState.js` | Ноды/offset/scale холста + вся локальная персистентность: черновик нод в localStorage (ключ — `canvasStorageKeys.js`, чистится в `CanvasPage.handleSave` после сохранения), сверка черновика с сервером при монтировании (если короче версии сервера — спрашивает подтверждение, а не молча подменяет), память позиции обзора, оповещение `onNodesChange` наружу. Вынесено из `CanvasBoard.jsx` |
...
| `NodeTextHighlighter.jsx` | Модальный редактор выделений текста: кликабельные токены, режимы «Плашка»/«Цвет текста», опакити, последние 5 цветов (localStorage), избранные (Supabase), предпросмотр в пузыре; виды выделения: цвет текста, плашка (заливка или обводка), жирный, подчёркивание, зачёркивание |
...
| `NodeTextTools.jsx` | Кнопки над текстом ноды: смайлики (у любого типа с текстом), раскраска слов и свои переносы (когда текст написан) |
| `NodeTextModals.jsx` | Окна над текстом ноды: раскраска, переносы, смайлики — вынесены из NodeContentEditor.jsx (потолок размера) |
| `useNodeEmoji.js` | Вставка смайлика в позицию курсора со сдвигом раскраски текста |
