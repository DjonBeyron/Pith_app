...
| `ProductionList.jsx` | Список по плану `buildRenderPlan`: обычно строка за строкой по центру (узкая колонка), но у ноды с развилкой (например word_choice) следующая по «верно» и цель «неверно» рисуются ПАРОЙ на всю ширину экрана — и остаются параллельными колонками, пока у ОБЕИХ сторон есть своё продолжение (несколько пар подряд), не разъезжаются в общий список после первого шага; каждая полноценная редактируемая ProductionRow, не превью. У каждой колонки своя кнопка «Добавить ноду ниже» (insertNodeAfter — к какой из двух веток цепляем, выбирает админ), между ними — маленькая «слить в одну» (insertNodeAfterBoth). Drag-хендл для смены порядка (во время протяжки — линия, куда встанет) |
| `ProductionRow.jsx` | Одна карточка ноды (тонированный фон по цвету типа — colorBg из nodeTypes.js, как в canvas; бар цвета, шапка с хендлом/номером/типом/кнопками, NodeContentEditor.jsx) — вынесена из ProductionList.jsx, переиспользуется и для обычной строки, и для каждой из двух колонок при развилке |
| `InsertNodeButton.jsx` | Кнопка «+ Добавить ноду...» — вместо того чтобы молча создавать ноду прошлого типа, открывает компактное меню выбора типа (тот же список/стиль и позиционирование — computeMenuPos, — что NodeTypeSelect.jsx в canvas). `branchChoices` — у кнопки на самой ноде-развилке после выбора типа второй шаг «к какому ответу присоединить» (не молчаливо к «верно») |
| `nodeGraphPrimary.js` | Графовые хелперы списка: `getPrimaryTriggerIndex`/`getBranchTriggerIndex`/`relinkPrimaryChain` (для drag-реордера) — переставляет «основной» триггер и x/y нод по новому порядку; `getPrimaryTarget`/`getBranchTarget` — куда ведут основной и «ветка»-триггеры; `buildRenderPlan` — план рендера (одиночные строки / пары при развилке, продолжает пары вниз пока обе стороны идут своим триггером — останавливается на тупике, слиянии или вложенной развилке; у single-ноды перед парой — `branchChoices` для InsertNodeButton); `insertNodeAfter` (опциональный `triggerIdx` — в какой из двух триггеров вставлять)/`insertNodeAfterBoth` (точка схождения — новая нода становится продолжением сразу и «верно», и «неверно»)/`insertNodeBefore`/`insertNodeAtStart` — точечная вставка ноды после/перед КОНКРЕТНОЙ (не по позиции в общем списке — у развилки её «верно»-ветка может иметь собственное продолжение, не соседнее с «неверно» по seq, поэтому вставка не трогает связи чужих нод) |
