...
| `CaptchaBox.jsx` | Виджет капчи Turnstile + подсказки вокруг него («подтверди, что ты не робот», «пробуем ещё раз», «не отвечает — продолжаем без неё» + кнопка «Повторить»). Ничего не рисует, пока форма не попросила токен через `useCaptcha().guard()` |
