...
| `SlideVideo.jsx` | Видео-слой слайда (общий для рекомендаций и «Моих уроков»): active приходит от родителя из позиции скролла (без IntersectionObserver); рендерит постер/скелетон/чип звука/паузу, а сам единый `<video>` (sharedVideoElement) переносит к себе, пока активен |
| `sharedVideoElement.js` | Единый переиспользуемый `<video>` на всю ленту + «парковка» вне слайдов. Один blessed-элемент вместо `<video>` на слайд: сохраняет разблокировку автозвука браузером (звук не гаснет через пару видео) и убирает дёрганье от пересоздания узлов |
| `useDragDebug.js` | Хук-инструментатор: вешает touch-слушатели на контейнер скролла и пишет в лог DBG смещения жеста (dx/dy пальца), паразитный горизонтальный скролл и реальный сдвиг видео за жест — для диагностики «дёрганья» при перетаскивании |
