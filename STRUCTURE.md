...
| Файл | Зачем нужен |
|------|-------------|
| `CanvasPage.jsx` | Обёртка страницы: шапка с кнопками «⚙», «← Назад», «▶» (открывает плеер), «↻ С сервера», «Очистить»/«В начало» (дотягиваются до CanvasBoard через ref), тройка «Сохранить → Граф → Продакшен»; запускает `LessonPlayer` поверх редактора |
...
| Файл | Зачем нужен |
|------|-------------|
| `ProductionPage.jsx` | Обёртка страницы: шапка (← Назад, название, «Очистить»/«В начало» (скроллит ProductionList к началу через scrollRef), тройка «Сохранить → Граф → Продакшен»), грузит/сохраняет тот же `lessons.script`, что и CanvasPage.jsx (сохраняет script целиком, не только nodes — не теряет lessonXp/учителя); при сохранении чистит localStorage-черновик canvas для этого урока (иначе canvas показал бы старый черновик вместо свежего порядка) |
