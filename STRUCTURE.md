...
| `feed-bubble-spoiler.css` | Стили спойлера-шариков фразы: обёртка, канвас шариков, текст фразы под ними (логика/анимация — `PhraseBubbleSpoiler.jsx`) |
...
| Файл | Зачем нужен |
|------|-------------|
| `FeedTab.jsx` | Лента: опубликованные модули из `curricula`, бесконечный scroll-snap круг с тихой перецентровкой, виртуализация TanStack (в DOM — видимый слайд + overscan 2); «Изучить фразу» открывает CurriculumView; deep-link репоста `/?m=<id>` — лента начинается с расшаренной фразы |
...
| `PhraseBubbleSpoiler.jsx` | Спойлер фразы: сетка мягких шариков без чёткой границы на canvas плавает поверх текста, тап — короткий взрыв (частицы разлетаются и гаснут), активен только на активном слайде (не тратит кадры на соседей) |
