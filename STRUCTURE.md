...
| `TabsNav.jsx` | Навигация по вкладкам: ряд кнопок на широком экране, бургер-меню на узком (≤620px) |
...
| `LessonSummary.jsx` | Экран итогов урока: анимированный XP-прогресс-бар, canvas-конфетти при повышении уровня, кнопка закрыть |
...
| Файл | Зачем нужен |
|------|-------------|
| `LessonsTab.jsx` | Оркестратор вкладки: список модулей → граф модуля → плеер/редактор; авто-создаёт 3 урока при открытии нового пустого модуля |
| `CurriculaList.jsx` | Список модулей: строки с названием, датой, кнопками открыть/переименовать/удалить |
| `useCurricula.js` | Хук: CRUD модулей в localStorage (id, title, createdAt) |
...
| `MgFinalNode.jsx` | Нод «Финал»: пятиугольник с детальным замком, XP-прогрессом и церемонией открытия (замок трясётся, открывается, блок вспыхивает deep-glow) |
...
| `LessonMapCanvas.jsx` | Устаревший canvas-граф (больше не используется, не удалён) |
...
| Файл | Зачем нужен |
|------|-------------|
| `ProfileTab.jsx` | Вкладка «Профиль»: аватар, прогресс-бар XP, текущий уровень, статистика (XP, стрик, уроки), карта всех уровней; пока на заглушках |
...
| `profileApi.js` | getProfile (чтение профиля) / completeLesson(lessonId) — начисление XP через RPC `complete_lesson` / resetLessonProgress(ids, clearAnswers) — сброс прохождения с возвратом XP через RPC `reset_lesson_progress` (тест-кнопки админа) |
