...
| Файл | Зачем нужен |
|------|-------------|
| `ProductionPage.jsx` | Обёртка страницы: шапка (← Назад, название, «Сформировать урок» → открывает тот же урок в canvas, «Сохранить»), грузит/сохраняет тот же `lessons.script`, что и CanvasPage.jsx (сохраняет script целиком, не только nodes — не теряет lessonXp/учителя) |
| `ProductionList.jsx` | Сам список: строки по seq сверху вниз, drag-хендл для смены порядка, «+» вставка между строками, дублировать/удалить, Ctrl+Enter — новая нода того же типа под текущей; содержимое строки — NodeContentEditor.jsx (тот же, что и canvas) |
| `nodeGraphPrimary.js` | Конвенция «основной путь» цепочки для списка: `getPrimaryTriggerIndex`/`relinkPrimaryChain` — при перетаскивании строки переставляет только «основной» триггер (played/timer или correct-ветку пары), branch-триггеры (неверно) не трогает |
