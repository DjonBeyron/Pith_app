...
| `ProductionRow.jsx` | Одна карточка ноды (бар цвета типа, шапка с хендлом/номером/типом/кнопками, NodeContentEditor.jsx) — вынесена из ProductionList.jsx, переиспользуется и для обычной строки, и для каждой из двух колонок при развилке |
| `nodeGraphPrimary.js` | Графовые хелперы списка: `getPrimaryTriggerIndex`/`relinkPrimaryChain` (для drag-реордера) — переставляет «основной» триггер и x/y нод по новому порядку; `getPrimaryTarget`/`getBranchTarget` — куда ведут основной и «ветка»-триггеры; `buildRenderPlan` — план рендера (одиночные строки / пары при развилке); `insertNodeAfter`/`insertNodeAtStart` — точечная вставка ноды после КОНКРЕТНОЙ (не по позиции в общем списке — у развилки её «верно»-ветка может иметь собственное продолжение, не соседнее с «неверно» по seq, поэтому вставка не трогает связи чужих нод) |
