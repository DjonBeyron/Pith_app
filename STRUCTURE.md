...
| `ProductionList.jsx` | Сам список: строки по seq сверху вниз, drag-хендл для смены порядка (во время протяжки видно линию, куда встанет нода), «+» вставка отдельным элементом МЕЖДУ строками, дублировать/удалить, Ctrl+Enter — новая нода того же типа под текущей; содержимое строки — NodeContentEditor.jsx (тот же, что и canvas); у нод с развилкой — вторая колонка (BranchPreview.jsx) |
| `nodeGraphPrimary.js` | Конвенция «основной путь» цепочки для списка: `getPrimaryTriggerIndex`/`relinkPrimaryChain` — при смене порядка переставляет «основной» триггер (played/timer или correct-ветку пары) и x/y всех нод в одну строку по новому порядку, чтобы canvas не путал линии; branch-триггеры (неверно) не трогает. `getBranchTarget`/`previewNodeText` — данные для BranchPreview.jsx |
| `BranchPreview.jsx` | Карточка ветки сбоку от ноды-развилки (например word_choice: верно/неверно) — тип, номер, короткий текст цели и кнопка «перейти к ноде» (скролл+подсветка в этом же списке, без дублирования редактора) |
