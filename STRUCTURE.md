...
| Файл | Зачем нужен |
|------|-------------|
| `FeedTab.jsx` | Лента (оболочка): загрузка опубликованных модулей из `curricula`, сплэш-релиз по первому кадру видео, вкладки Рекомендации/Мои уроки, рендер слайдов; механика — в хуках useFeedLoop/useFeedSound/useFeedSocial; «Изучить фразу» открывает CurriculumView |
| `useFeedLoop.js` | Хук бесконечного круга ленты: виртуализация TanStack (видимый слайд + overscan 2), телепорт scrollTop с выключенным snap, перецентровка у края запаса, активный слайд из позиции скролла, высота вьюпорта |
| `useFeedSound.js` | Хук звука ленты: soundOn в localStorage, жест пользователя (чип), откат при блокировке автозвука iOS, итоговый soundReady |
| `useFeedSocial.js` | Хук соц-действий ленты: лайки/закладки (оптимистично + сервер в фоне), серверные счётчики, начатые модули (refreshStarted), гостя отправляет в форму входа |
