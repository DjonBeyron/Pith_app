...
| Файл | Зачем нужен |
|------|-------------|
| `ProductionPage.jsx` | Обёртка страницы: шапка (← Назад, название, тройка «Сохранить → Граф → Продакшен»), грузит/сохраняет тот же `lessons.script`, что и CanvasPage.jsx (сохраняет script целиком, не только nodes — не теряет lessonXp/учителя); при сохранении чистит localStorage-черновик canvas для этого урока (иначе canvas показал бы старый черновик вместо свежего порядка) |
