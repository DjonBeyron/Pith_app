...
| `FeedSlide.jsx` | Один слайд: видео-слой SlideVideo, фраза под спойлером-шариками (PhraseBubbleSpoiler, тап разлетает), HUD (лайк со счётчиком/закладка — сервер, репост через Web Share), зона замедления 0.5x над лайком (useSlowMotion), кнопка «Изучить фразу» |
...
| `MyLessonSlide.jsx` | Один слайд видео-режима «Моих уроков»: видео-слой SlideVideo, зона замедления 0.5x над HUD (useSlowMotion, тот же зона-класс, что в ленте), бейдж сложности, кнопка «Продолжить»/«Повторить модуль», прогресс-бар модуля |
