...
| Файл | Зачем нужен |
|------|-------------|
| `UserTab.jsx` | Показывает файлы по одному каждые 3 сек (имитация чата), номер по порядку загрузки, проигрывает фото/видео/аудио; держит в памяти только последние 5 — остальные выгружает |
| `useSequentialPreload.js` | Качает файлы по одному, строго по очереди (не параллельно), с авто-повтором при обрыве; считает скорость каждого; умеет выгружать («evict») готовый файл из памяти по команде — всё пишет в дебаг-лог |
