...
| `feed-v2.css` | Стили ленты видео (ui v2): scroll-snap слайды, градиенты-заглушки, верхние вкладки, HUD, кнопка «Изучить фразу» с бликом, спойлер, тост |
...
| `FeedSlide.jsx` | Один слайд: видео-слой SlideVideo, фраза под спойлером (blur+зерно, тап раскрывает), HUD (лайк со счётчиком/закладка — сервер, репост через Web Share), кнопка «Изучить фразу» |
...
| Файл | Зачем нужен |
|------|-------------|
| `useRaceState.js` | Хук состояния гонки: актуальная гонка, задания-МОДУЛИ с прогрессом, ПРО-модуль (супер-урок), моя запись, фаза, порог 80% XP; + weekKey и ключ «модуль пройден на неделе» |
