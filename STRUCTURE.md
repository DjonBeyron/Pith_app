...
| `InsertNodeButton.jsx` | Кнопка «+ Добавить ноду...» — вместо того чтобы молча создавать ноду прошлого типа, открывает компактное меню выбора типа (тот же список/стиль, что NodeTypeSelect.jsx в canvas). `branchChoices` — у кнопки на самой ноде-развилке после выбора типа второй шаг «к какому ответу присоединить» (не молчаливо к «верно») |
