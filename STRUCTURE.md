...
| `nodeGraphPrimary.js` | Графовые хелперы списка: `getPrimaryTriggerIndex`/`relinkPrimaryChain` — при смене порядка переставляет «основной» триггер и x/y всех нод в одну строку, чтобы canvas не путал линии; `getPrimaryTarget`/`getBranchTarget` — куда ведут основной и «ветка»-триггеры; `buildRenderPlan` — строит план рендера списка (одиночные строки / пары при развилке), каждая нода рисуется ровно один раз |
