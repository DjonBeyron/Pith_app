...
// Свободное место под новую ноду рядом с исходной.
//
// Раньше вставка расталкивала соседей: всё, что правее, уезжало на слот вправо.
// На собранном графе это перекраивало расстановку целиком — автор терял свою
// раскладку из-за одной новой ноды. Теперь никого не двигаем: ставим ноду на
// желаемое место, а если оно занято — спускаемся ниже, пока не найдём пустое.
const GAP_X = NODE_SLOT * 0.8
const GAP_Y = NODE_ROW * 0.6

export function findFreeSpot(nodes, x, y) {
  const taken = ny => (nodes ?? []).some(n =>
    Math.abs((n.x ?? 0) - x) < GAP_X && Math.abs((n.y ?? 0) - ny) < GAP_Y)
  let ny = y
  // Потолок на всякий случай: пустое место найдётся задолго до него
  for (let i = 0; i < 60 && taken(ny); i++) ny += NODE_ROW
  return { x, y: ny }
}
