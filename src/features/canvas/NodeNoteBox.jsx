...
//
// Это заметка «для своих»: что доснять, чей голос, какой дубль взять. Текст
// живёт в самой ноде (node.note) и уезжает вместе с уроком, а вот где стикер
// стоит на экране (box) — личное дело того, кто сейчас в редакторе: это
// положение хранится в localStorage (useLocalNoteBox.js), а не в уроке.
// Заметка НИКОГДА не показывается ученику — ни в плеере, ни в чате: плеер
// поле note просто не читает. Видит её только админ и только в канвасе.
//
// Стикер можно таскать за шапку и растягивать за любую из восьми ручек. Пока
// его не двигали, box нет — он стоит над нодой на месте по умолчанию.
