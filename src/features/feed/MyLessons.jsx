...
export default function MyLessons({
  visible = true, modules, startedIds, bookmarkedLessons, onOpen, onOpenLesson, onGoFeed,
  ...
  // своего модуля вокруг (StandaloneLessonRunner.jsx). Приходят пропсом из
  // FeedTab: грузятся вместе с модулями, а не при открытии вкладки
  // (см. useBookmarkedLessons.js).
  ...
  useEffect(() => {
    ...
    let cancelled = false
    ;(async () => {
      // Сами закладки грузятся вместе с лентой (useBookmarkedLessons) — здесь
      // остались только чекпойнты: проценты недопройденного и пересдач
      const bmIds = bookmarkedLessons.map(l => l.id)
      ...
      {mode === 'video' ? (
        <div className="feedV2Scroll" ref={scrollRef} onScroll={onScroll}>
          ...
          {visibleLessons.map(l => (
            <MyLessonRefSlide key={l.id} lesson={l} onOpen={() => onOpenLesson(l.id, l.title)} />
          ))}
        </div>
      ) : (
        <div className="mlList">
          ...
          {visibleLessons.map(l => (
            <div key={l.id} className="mlRowShell">
              <button className="mlRow" onClick={() => onOpenLesson(l.id, l.title)}>
