...
export default function SlideVideo({
  ...
  useEffect(() => {
    ...
    // Флаг для пула: только активный слайд можно будить play() из отложенных
    // колбэков (перезапуск загрузки, пинок поверхности, возврат из фона).
    // Без него сторож зависшей загрузки, чей перезапуск доехал через 5с,
    // включал видео слайду, который к тому моменту стал соседом: два видео
    // декодировались одновременно, невидимое — с opacity 0 (датчик на iPhone:
    // video=4/2, второе с top=-812)
    v.dataset.active = active ? '1' : ''
    if (!active) {
