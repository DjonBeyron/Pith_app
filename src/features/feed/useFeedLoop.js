...
// Бесконечный круг ленты: список повторяется циклами, а при подходе к краю
// scrollTop незаметно переносится на один цикл внутрь (контент идентичен —
// скачка не видно). Здесь же виртуализация TanStack и активный слайд.
export function useFeedLoop(len) {
  ...
  useEffect(() => {
    ...
    apply()
  }, [len, cycles, viewH]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return { scrollRef, activeIdx, viewH, cycles, virtualizer, onScroll }
}
