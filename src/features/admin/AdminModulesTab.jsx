...
import { loadCurricula, saveCurriculum, deleteCurriculumFromServer, updateCurriculumStatus } from '../../shared/lib/curriculaApi.js'
...
import ModuleTitleEditor from './ModuleTitleEditor.jsx'
...
export default function AdminModulesTab({ onOpenCanvas, onOpenProduction }) {
  ...
  // Модуль, у которого открыт редактор названия и переводов (ModuleTitleEditor)
    ...
    <div className="amScreen">
      ...
      {rows === null ? (
        ...
        <div key={m.id} className="amRow">
          ...
          <button
            className="amChip amChipDim amChipBtn"
            onClick={() => setTitleEdit({ id: m.id, title: m.title })}
            title="Название и перевод">✎</button>
      ...
      {titleEdit && (
        <ModuleTitleEditor
          moduleId={titleEdit.id}
          initialTitle={titleEdit.title}
          onClose={() => setTitleEdit(null)}
          onSaved={t => setRows(rs => rs.map(r => r.id === titleEdit.id ? { ...r, title: t } : r))}
        />
      )}
    </div>
  )
