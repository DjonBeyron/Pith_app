...
export default function PinMessageModule({ node, teacherName, onDone }) {
  ...
  useEffect(() => {
    ...
    // в логе pin-message честно играл двумя копиями подряд. Восстановленная
    // история («Продолжить урок») — тоже без звука, сообщение уже закреплялось
    ...
    const t = pending || node?.isHistory
      ? null : setTimeout(() => playSound('pin-message', 'закреплённое сообщение'), MSG_SOUND_AT)
    onDone?.()
