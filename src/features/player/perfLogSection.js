...
// сотен строк плеера, а смотреть их надо столбиками подряд: рос ли inf/waves
...
export function buildPerfSection(playerLines) {
  ...
  let minFps = Infinity, drops = 0, maxInf = 0, maxWaves = 0, maxAnim = 0, maxAudio = 0, frozen = 0
  for (const l of rows) {
    ...
    maxWaves = Math.max(maxWaves, num(l, 'waves') ?? 0)
  ...
  return [
    ...
    `max: anim=${maxAnim} inf=${maxInf} waves=${maxWaves} audio=${maxAudio}`,
    ...rows,
  ]
}
