...
export function decorStyle(span) {
  ...
  return {
    ...
    ...(lines.length ? {
      ...
      // Было 0.055em — заметно толще обычного веб-подчёркивания, спорило
      // с самими буквами. Тоньше примерно в 1.8 раза — линия читается как
      // акцент, а не как отдельная плашка под текстом.
