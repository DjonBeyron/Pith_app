...
// Чуть-чуть «внутрь» последнего кадра композиции: конец клипа исключающий
// (t < end), поэтому ровно end читался бы как «уже за клипом».
const EPS_S = 0.001
...
export function computeRevealedCellIds(layers, t) {
  // Аудио может оказаться ДЛИННЕЕ композиции: автор укоротил таймлайн или
  // перегенерировал озвучку, а длина композиции осталась прежней. rAF плеера
  // считает кадры по audio.currentTime, пока звук не кончился, и время уходит
  // за конец ВСЕХ клипов таймлайна. Авторского указания на этот хвост нет — а
  // «указания нет» и «спрятать» это не одно и то же. Раньше выходило именно
  // второе: клип проявления по умолчанию растянут ровно на композицию, за её
  // концом он больше не покрывает t, и текст ВСЕХ ячеек гас в 0. На живом
  // уроке (таймлайн 6.0с, озвучка 8.87с) таблица стояла пустой 2.8 секунды,
  // причём гасли и уже разобранные ячейки, которым положено 40% прозрачности.
  // Держим последний кадр композиции: что было видно в её конце — то и видно.
  const end = timelineEndSec(layers)
  const at = end > 0 && t > end ? end - EPS_S : t
  ...
  for (const l of layers ?? []) {
    ...
    if (at >= reveal.start && at < reveal.end) revealed.add(l.cellId)
  }
